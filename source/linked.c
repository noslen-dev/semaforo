...
/****
 * static void free_head_and_nodes(ponteiro para a cabeca da lista)
...
/****
 * void free_list_and_tab_in_head(ponteiro para a cabeca da lista, linhas do tabuleiro presente na cabeca da lista)
...
/*****
 * bool add_node_in_head(ponteiro para a cabeca da lista, linhas do tabuleiro de jogo atual, 
 * colunas do tabuleiro de jogo atual, nome do jogador atual, peca colocada, 
 * coordenadas em que a peca foi colocada)
...
/*****
 * bool add_node_to_node(ponteiro para a cabeca da lista,ponteiro para o no anterior, 
 * linhas do tabuleiro de jogo atual, colunas do tabuleiro de jogo atual, 
...
bool add_node_to_node(struct list_head *head,struct list_node *prev, int lin, int col, char name, char piece, struct coordinates place){
    ...
    if( ( new_node=malloc(sizeof(struct list_node)) )==NULL ){
        fprintf(stderr,ALLOCATION_ERROR_LINKED);
        free_list_and_tab_in_head(head, prev->lin); 
        return 0;
    }
    new_node->lin=lin; new_node->col=col;
    new_node->player_name=name;
...
/***
 * bool place_piece_in_head_tab(ponteiro para a cabeca da lista, no da lista)
...
/***
 * bool reset_tab_in_head(ponteiro para a cabeca da lista, linhas do tabuleiro presente na cabeca da lista)
...
/**
 * bool show_k_prev(numero de jogadas anteriores, ponteiro para a cabeca da lista, 
 * no que representa o turno atual do jogo)
